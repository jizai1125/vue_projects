      ...
      >
        ...
        <el-table-column label="文件名" sortable show-overflow-tooltip>
          <template slot-scope="scope">
            ...
            <span
              @click="folderEnter(scope.row)"
              :class="{ diskMain_name: scope.row.type === 'folder' }"
            >
              {{ scope.row.name }}
            </span>
          </template>
        ...
        <el-table-column width="70">
          <template slot-scope="scope">
            ...
            <span @click="download(scope.row)" v-show="scope.row.type == 'file'"
...
export default {
  ...
  methods: {
    ...
    getCloudList(id) {
      ...
      this.loading = true;
      http
        .getService("cloudlist?foldId=" + id)
        .then((res) => {
          var data = [];
      ...
      MessageBox.prompt("", "修改名称", {
        ...
        .then(({ value, is }) => {
          http
            .postService(
              "updatefile",
              JSON.stringify({ name: value, _id: node._id, type: node.type })
            )
            .then((res) => {
              if (res.status == "success") {
    ...
    download(file) {
      ...
      link.setAttribute(
        "href",
        `http://localhost:3000/api/downloadfile?fileId=${file._id}&skyAuth=aut${
...
.diskMain .el-icon-edit,
.diskMain .el-icon-download {
  color: #409eff;
  cursor: pointer;
  padding: 0 5px;
}
