...
export default {
  ...
  methods: {
        ...
        .then(({ value }) => {
          http
            .postService(
              ...
              JSON.stringify({
                foldname: value,
                parId: this.curFoldId,
                pathRoot: this.pathRoot.concat(this.curFoldId),
              })
            )
            .then((res) => {
    ...
    deleteList: function() {
      //删除操作
      MessageBox.confirm(
        "此操作将永久删除文件(若删除文件夹，则该文件夹的所有数据都会一并删除)，是否继续?",
        "提示",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
        ...
        .then(() => {
          ...
          http
            .postService(
              "deletelist",
              JSON.stringify({ deletelist: deleteArr })
            )
            .then((res) => {
              if (res.status == "success") {
    ...
    confirmMove: function() {
      ...
      http
        .postService(
          "movelist",
          JSON.stringify({
            targetId: this.hasClickNode._id,
            targetPath: this.hasClickNode.pathRoot,
            movelist: moveArr,
            curPath,
          })
        )
