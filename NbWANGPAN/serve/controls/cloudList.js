...
const uploadFile = async (ctx, next) => {
  ...
  const creator = ctx.get("authId").slice(3);
  //校验数据库中是否已存在同名
  let validRes = await validSameName({
...
const addFold = async (ctx, next) => {
  ...
  const creator = ctx.get("authId").slice(3);
  if (foldname && foldname.match(/^[(\u4e00-\u9fa5)|(a-zA-Z0-9)]+$/)) {
    //为保证可靠性后台需要校验
    //校验数据库中是否已存在同名
    let validRes = await validSameName({
...
const deletefile = async (ctx, next) => {
  ...
  let doc = await queryList({
    $or: [
      { _id: { $in: ctx.request.body.deletelist } },
      { pathRoot: { $in: ctx.request.body.deletelist } },
    ],
  });
  ctx.status = 200;
  if (!doc) {
...
const movefile = async (ctx, next) => {
  await next();
  //改变当前移动内容的parId
  let doc1 = await updateList(
    { _id: { $in: ctx.request.body.movelist } },
    { $set: { parId: ctx.request.body.targetId } }
  );
  ctx.status = 200;
  if (!doc1) {
    ctx.body = {
  ...
  let doc2 = await queryList({
    $or: [
      { _id: { $in: ctx.request.body.movelist } },
      { pathRoot: { $in: ctx.request.body.movelist } },
    ],
  });
  let doc;
...
const updatefile = async (ctx, next) => {
  ...
  let doc = await updateList(
    { _id },
    { $set: { name, updateTime: new Date().getTime() } }
  );
  ctx.status = 200;
  ctx.body = {
    status: doc ? "success" : "error",
...
const downloadfile = async (ctx, next) => {
  ...
  if (doc && doc.length > 0) {
    ...
    ctx.set(
      "Content-disposition",
      `attachment;filename=${encodeURI(doc[0].name)}`
    ); //涉及到中文需要编码
