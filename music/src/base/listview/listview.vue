<template>
  ...
  >
    <!-- 歌手列表 -->
    <ul>
      <li v-for="group in data" :key="group.tag" ref="listGroup" class="list-group">
        <h2 class="list-group-title">{{ group.tag }}</h2>
        <ul>
          <li v-for="item in group.data" :key="item.singer_mid" class="list-group-item" @click="selectItem(item)">
