...
export default {
  ...
  watch: {
    currentSong(newSong, oldSong) {
      if (!this.showFlag || newSong._id === oldSong._id) return
  ...
  methods: {
    ...
    scrollToCurrent(current) {
      console.log(current)
      console.log('list item', this.$refs.listItem)
      this.$nextTick()
        .then(() => {
          ...
          const currentEleIndex = this.$refs.listItem.findIndex(item => {
            return item.dataset.id === current._id.toString()
          })
          console.log(currentEleIndex)
