<template>
  <div v-show="playList.length" class="player">
    ...
    >
      ...
      <div v-show="fullScreen" class="normal-player">
        ...
        <div class="background">
          <img width="100%" height="100%" :src="'http://localhost:3000'+ currentSong.singer.pic">
        </div>
        <!-- 返回按钮 -->
        <div class="top">
          ...
          <h1 class="title" v-html="currentSong.title" />
          <h2 class="subtitle" v-html="currentSong.singer.name" />
        </div>
        <div
          class="middle"
        ...
        >
          ...
          <div class="middle-l">
            <div ref="cdWrapper" class="cd-wrapper">
              <div v-if="currentSong" :class="cdRoll" class="cd">
                <img :src="'http://localhost:3000'+ currentSong.singer.pic" alt class="image">
              </div>
            </div>
      ...
      <div v-show="!fullScreen" class="mini-player" @click="setFullScreen(true)">
        <div class="icon">
          <img
            ...
            width="40"
            height="40"
            :src="'http://localhost:3000' + currentSong.singer.pic"
            alt
          >
        </div>
        <div class="text">
          <h2 class="name" v-html="currentSong.title" />
          <div class="desc" v-html="currentSong.singer.name" />
        </div>
        <div class="control">
    ...
    <audio
      ref="audio"
      :src="'http://localhost:3000'+ currentSong.url"
...
export default {
  ...
  watch: {
    currentSong(newSong, oldSong) {
      ...
      if (newSong._id === oldSong._id) return;
      this.currentLyric && this.currentLyric.stop();
      // this.$nextTick(() => {
      setTimeout(() => {
        this.$refs.audio.play();
        this.songReady = true;
        this._getLyric(newSong._id);
      }, 1000);
      // })
    },
    playing(newStatus) {
  ...
  methods: {
    ...
    _getLyric(mid) {
      ...
      getLyric(mid)
        .then(res => {
          ...
          this.currentLyric = new Lyric(res.content, this.handleLyric);
          if (this.playing) {
    ...
    middleTouchMove(e) {
      ...
      const offsetLeft = Math.min(
        0,
        Math.max(-window.innerWidth, diffX + left)
      );
    ...
    error() {
      this.songReady = true;
      // this.currentTime = 0
      console.error("<<<song load error>>>");
    },
    // 切换播放状态
    togglePlaying() {
      if (!this.songReady && !this.currentLyric) return;
